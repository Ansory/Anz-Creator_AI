...
HANYA JSON, tanpa teks lain. Jangan pakai emoji di text."""
    ...
    def _write_srt(self, scenes: List[Scene], path: Path) -> None:
        ...
        with open(path, "w", encoding="utf-8") as f:
            idx = 1
            for sc in scenes:
                # PERBAIKAN: Pecah 1 paragraf panjang menjadi potongan 4-6 kata
                words = sc.text.split()
                chunk_size = 5 # Maksimal 5 kata per layar
                chunks = [" ".join(words[i:i+chunk_size]) for i in range(0, len(words), chunk_size)]
                
                if not chunks:
                    chunks = [sc.text]
                    
                # Estimasi sinkronisasi waktu agar pas dengan durasi audio scene
                time_per_chunk = sc.duration / len(chunks)
                
                for chunk in chunks:
                    start = t
                    end = t + time_per_chunk
                    f.write(f"{idx}\n{ts(start)} --> {ts(end)}\n{chunk}\n\n")
                    idx += 1
                    t = end
